	...
	def get_devices(self):
		...
		for item in list_output:
			...
			if aux_splited.group(2) == 'disk':
				...
				list_output_mountpointsdevice=[x.decode('utf-8') for x in cmd1.stdout.readlines()]
				mountpointsdevice = list(map(str.strip,list_output_mountpointsdevice))
				print(mountpointsdevice)
				found = False
	...
	def rescuepxe_cdrom_test(self,DEVICE):
		...
		try:
			...
			if os.path.dirname(DEV_FILE):
				...
				output=s.check_output(instruction, shell=True)
				print(output)
				x='cd_r'
				if (x in output):
